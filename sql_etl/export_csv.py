...
class DataExporter:
    ...
    def export_and_rotate(self) -> Optional[str]:
        """Export old data to CSV and delete from database."""
        ...
        try:
            with self.db_manager.get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    # Export to CSV
                    filename = self._export_to_csv(rows)
                    ...
                    logger.info(f"Deleted {deleted_count} rows older than {self.config.config['interval_minutes']} minutes")
    ...
    def _export_to_csv(self, rows: List[Tuple]) -> str:
        """Export rows to CSV file."""
        timestamp_str = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
        filename = self.export_dir / f"export_{timestamp_str}.csv"
        
        headers = ['timestamp', 'component', 'status', 'db', 'topfreq1', 'topfreq2', 'topfreq3', 'tester_id']
    ...
    def cleanup_old_exports(self):
        ...
        try:
            for file_path in self.export_dir.glob("export_*.csv"):
